...
| # | Case | Expectation |
| --- | --- | --- |
| 1 | New game with no state | White to move, version 0, empty history |
| 2 | Assignment with uneven populations | Smaller side chosen |
| 3 | Assignment with equal populations | Side to move chosen |
| 4 | Two submissions at the same version | Exactly one returns `ok: true` |
| 5 | Stale version | `superseded` |
| 6 | Wrong side | `not_your_side` |
| 7 | Version checked before side | Stale + wrong side yields `superseded` |
| 8 | Illegal move per rules | `illegal_move` |
| 9 | Accepted move | Version increments by exactly 1 |
| 10 | Same session moving twice | `contributors` unchanged |
| 11 | New session moving | `contributors` increments |
| 12 | Repetition via `positionKeys` | Draw outcome reported |
| 13 | Just before `stallAfterMs` | Wrong side still rejected |
| 14 | Just after `stallAfterMs` | Any side accepted, `stallOpen: true` |
| 15 | Move after game over | `game_over` |
| 16 | `serialize()` then `createArbiter` | Lossless round trip |
| 17 | `publicView()` | No `seenSessions`; history capped at limit |
| 18 | Input state object | Unmutated after `submit` |

Cases 4, 5, and 7 are the ones the whole feature rests on. Write them first.

A separate, smaller suite covers the chess adapter: a full recorded game replays correctly, castling and en passant survive a serialize round trip, and checkmate reports the right winner.
...
| 1 | Toy rules + core arbiter | All 18 cases in §6 pass |
| 2 | Chess adapter on `./chess` | A recorded game replays end to end |
| 3 | Consumed by the portfolio via `file:` | Feature works locally |
| 4 | Publish `0.1.0` | Installable with provenance |
